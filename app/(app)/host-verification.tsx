...
type VerificationStatus = "idle" | "pending" | "approved" | "rejected" | "not_submitted";
...
export default function HostVerificationScreen() {
  ...
  const uploadIdImage = async (uri: string): Promise<string> => {
    ...
    const fileName = `verification/${user.id}/id_${Date.now()}.jpg`;
    const { data, error } = await supabase.storage
      .from("flyers")
      .upload(fileName, blob, { contentType: "image/jpeg", upsert: true });
    if (error) throw error;
    const { data: urlData } = supabase.storage.from("flyers").getPublicUrl(data.path);
    return urlData.publicUrl;
  };

  const handleSubmit = async () => {
    if (!user) return;
    if (!fullName.trim() || !idType.trim() || !idNumber.trim() || !idImageUri || !address.trim() || !phone.trim()) {
    ...
    try {
      ...
      const { error: upsertError } = await supabase.from("host_verifications").upsert(
        {
          user_id: user.id,
  ...
  return (
    ...
    >
      <View className="pt-16 px-6 pb-6 border-b border-white/5">
        <TouchableOpacity onPress={() => router.back()} className="w-10 h-10 mb-4">
      ...
      <ScrollView className="flex-1 px-6 pt-6" showsVerticalScrollIndicator={false}>
        {status === "pending" && (
          <View className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-4 mb-6">
            <Text className="text-amber-400 font-semibold">Verification pending</Text>
            <Text className="text-gray-300 text-sm mt-1">
              Your verification is under review. You'll be able to create parties once approved.
            </Text>
        ...
        {status === "rejected" && (
          <View className="bg-red-500/10 border border-red-500/30 rounded-2xl p-4 mb-6">
            <Text className="text-red-400 font-semibold">Verification rejected</Text>
            {rejectionReason && (
              <Text className="text-gray-300 text-sm mt-1">Reason: {rejectionReason}</Text>
            )}
            <Text className="text-gray-400 text-sm mt-2">You may resubmit with correct information.</Text>
          </View>
        )}
        ...
        {(status === "not_submitted" || status === "rejected") && (
          <>
            ...
            <View className="mb-4">
              ...
              >
                {idImageUri ? (
                  <ExpoImage source={{ uri: idImageUri }} className="w-full h-32 rounded-lg" contentFit="cover" />
                ) : (
                  <>
                    <Ionicons name="camera-outline" size={40} color="#666" />
                    <Text className="text-gray-400 mt-2">Tap to upload ID photo</Text>
                  </>
                )}
              </TouchableOpacity>
            </View>
