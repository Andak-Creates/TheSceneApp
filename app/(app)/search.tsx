...
export default function SearchScreen() {
  ...
  const fetchSearchParties = useCallback(async (query: string = "", genre: string | null = null) => {
    setLoading(true);
    try {
      ...
      <View className="flex-1 justify-center pr-4">
        ...
        <View className="flex-row items-center mt-2">
            <Text className="text-purple-400 font-bold text-sm">
              {item.date_tba ? "TBA" : item.date ? new Date(item.date).toLocaleDateString() : ""}
            </Text>
            <View className="w-1 h-1 rounded-full bg-gray-600 mx-2" />
            <Text className="text-white font-medium text-sm">
                {getCurrencySymbol(item.currency_code || "NGN")}{item.ticket_price?.toLocaleString() || "0"}
            </Text>
        </View>
      </View>
    </TouchableOpacity>
    ...
    <View className="flex-1 bg-[#09030e] pt-16">
      ...
      <View>
          ...
          >
              ...
              >
                <Text className={`font-bold text-xs ${!selectedGenre ? 'text-white' : 'text-gray-400'}`}>All</Text>
              </TouchableOpacity>
              {GENRES.map(genre => (
                  <TouchableOpacity
                    key={genre}
                  ...
                  >
                    <Text className={`font-bold text-xs ${selectedGenre === genre ? 'text-white' : 'text-gray-400'}`}>{genre}</Text>
                  </TouchableOpacity>
              ))}
          </ScrollView>
      </View>
        ...
        <FlatList
          ...
          refreshControl={
            <RefreshControl
              ...
              onRefresh={() => { setRefreshing(true); fetchSearchParties(searchQuery, selectedGenre); }}
              tintColor="#a855f7"
            />
          }
          ListEmptyComponent={
